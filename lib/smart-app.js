...
module.exports = class SmartApp {
    ...
    constructor(options = {}) {
        ...
        this._pages = {};
        this._installedHandler = ((ctx, evt) => { this._updatedHandler(ctx, evt.installData); });
    ...
    _handleCallback(evt, responder) {
        try {
            ...
            switch (evt.lifecycle) {
                ...
                case 'CONFIGURATION': {
                    const configurationData = evt.configurationData;
                ...
                case 'EVENT': {
                    this._log.event(evt);
                    for (let event of evt.eventData.events) {
                        switch (event.eventType) {
                            ...
                            case 'DEVICE_COMMANDS_EVENT': {
                                if (this._deviceCommandHandler) {
                                    ...
                                    const deviceCommandsEvent = event.deviceCommandsEvent;
                                    for (let cmd of deviceCommandsEvent.commands) {
            ...
            this._log.error(e.message ? e.message : e.toString());
        }
    }
};
